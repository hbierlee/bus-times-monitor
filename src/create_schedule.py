...
def inner_join_file_to_schedule(schedule, path, key):
	with open(path) as file:
		...
		other_data = sorted(other_data, key=sorting_key)  # should already be the case
		
		# TODO can be improved by just using zip from the index of the first trip_id match (I think?)
		otherIndex = 0;
		for bus in schedule:
			routeEntry = next({"other": other, "index": index} for index, other in enumerate(other_data) if other[key] == bus[key])
